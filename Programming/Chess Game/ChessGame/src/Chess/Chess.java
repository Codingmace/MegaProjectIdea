...
public class Chess {
    ...
    /**
     * Used during debug compilations. Most print-outs have been removed,
    ...
    /**
     * True while the AICaller thread is working. If it is falsed, the thread
     * will stop ASAP.
     */
    ...
    /**
     * The all-important movement table for generating possible moves for a
     * given piece. This is indexed via index.
     *
     * @see index
     */
    ...
    /**
     * Arbitrary piece values. Note how the king is indispensable - important
     * for checks.
     */
    ...
    /**
     * A temporary list of possible moves for a given piece. Contains a maximum
     * of 32 moves.
     */
...
//			int nodeCount
    ...
    /**
     * An integer to display the maximum depth reached while searching for a
     * move.
     */
    ...
    protected float alphaBetaHelper(int depth, ChessPosition p, boolean player, float alpha, float beta) {
        /**
         * This used to decrease speed by a magnitude of 5, but we're optimizing
         * it by only searching for checks if we're already in check, thus
         * finding only mates. Checks will register as 'king takes' and will
         * count as such.
         */
        ...
        /**
         * First, check whether the other player is in check due to previous
         * move
         */
        if (player ? p.bBlackChecked : p.bWhiteChecked) {
            ...
            /**
             * If the player making the move is still checked - he is
             * effectively mated, which must be avoided at all costs. This is
             * worse if it was his turn. If he is not checked any more, that is
             * OK. If
             */
            if (player ? p.bBlackChecked : p.bWhiteChecked)// && !(player ? p.bWhiteChecked : p.bBlackChecked) )
            {
                /**
                 * This forces the side to move out of check, despite seeing a
                 * riposting check(mate).
                 */
                if (depth == 2) {
     ...
     * assigns a floating-point value to it, guessing as to the chances of a win
     * arising from that position for a given side. It takes into account
     * material values (the pieces on the board) and calls a special
    ...
    /**
     * Generates all possible moves for a given piece (square_index) on a given
     * position (pos), storing the moves in piece_moves and returning the number
     * of available moves. This function is extensively called indirectly by the
     * alphaBetaHelper function.
     */
    ...
    /**
     * When the user decides to take back a move (or two), this function handles
     * restoring the board to its previous state in time and refreshing the move
     * list and graph.
     */
    void Takeback() {
    ...
    /**
     * The following routine cycles through all the pieces on the board,
     * calculating moves for each piece
     */
    ...
    /**
     * Data to store board control for each player, used when evaluating board
     * position This data is static that can be re-used (assume single
     * threading!) Used by setControlData
     */
    ...
    /**
     * This function calculates positional advantage for control of the chess
     * board. It also eats up most of our CPU time.
     */
    ...
    /**
     * An index for determining the text values of pieces for position
     * encoding/decoding.
     *
     * @see encodePosition
     * @see decodePosition
     ...
     * the value 6, returns a 'k' for king, or if -6, a 'K' for a black king.
